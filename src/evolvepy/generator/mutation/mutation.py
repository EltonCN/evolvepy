...
class BinaryMutationLayer(ChromossomeOperator):
    def __init__(self, mutation_function:Callable, existence_rate:float, gene_rate:float, name: str = None, chromossome_names: Union[str, List[str], None] = None):
    ...
    def call_chromossomes(self, chromossomes: np.ndarray, fitness:np.ndarray, context:Context, name:Optional[str]) -> np.ndarray:
        ...
        return BinaryMutationLayer.mutate(chromossomes, self._mutation_function, existence_rate, gene_rate)

    @staticmethod
    @numba.njit()#parallel=True)
    def mutate(chromossomes:np.ndarray, mutation_function:Callable, existence_rate:float, gene_rate:float):
        result = np.empty_like(chromossomes)

        n = chromossomes.shape[0]
        for i in numba.prange(n):
            result[i] = mutation_function(chromossomes[i], existence_rate, gene_rate)
