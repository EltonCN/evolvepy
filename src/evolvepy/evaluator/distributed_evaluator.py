...
class DistributedFitnessFunction(ABC):
    ...
    def start_evaluation(self, individuals:np.ndarray) -> np.ndarray:
        '''
...
def evaluate_forever(fitness_function: Type[DistributedFitnessFunction], individuals_queue: ray.Queue, scores_queue: ray.Queue, args: Dict[str, object]):
    ...
    while True:
        ...
        scores = ray.get(evaluator(individuals))
        scores_queue.put.remote((scores, first, last))
...
class DistributedEvaluator(Evaluator):
    ...
    def __init__(self, fitness_function: Type[DistributedFitnessFunction], n_process: int = None, timeout: int = None, n_scores: int = 1, individual_per_call: int = 1, args: Dict[str, object] = None) -> None:
        '''
        ProcessEvaluator constructor.

        Args:
            fitness_function (Type[ProcessFitnessFunction]): Class that will be used to evaluate individuals.
            n_process (int, optional): Number of processes to use. Defaults to None (same number as cpu_count).
            timeout (int, optional): Maximum time to wait for a new evaluation. Defaults to None (infinity).
            n_scores (int, optional): Number of scores generated by the fitness function. Defaults to 1.
    ...
    def __call__(self, population: np.ndarray) -> np.ndarray:
        ...
        Raises:
            RuntimeError: Raised if the population size is not divisible by individual_per_call.
        ...
        if n % self._individual_per_call != 0:
            raise RuntimeError("Population size must be divisible by individual_per_call")
