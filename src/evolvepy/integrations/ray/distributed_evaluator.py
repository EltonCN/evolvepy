...
class DistributedEvaluator(Evaluator):
    def __init__(self, fitness_function: Type[ProcessFitnessFunction], n_worker:int=None, n_scores: int = 1, individual_per_call: int = 1, args:Dict[str, object]=None, name:str=None):
        ...
        other_parameters={"evaluation_function_name":fitness_function.__name__, "n_worker":n_worker}
        super().__init__(n_scores=n_scores, individual_per_call=individual_per_call, name=name, other_parameters=other_parameters)
    ...
    def call(self, individuals: np.ndarray) -> np.ndarray:
        ...
        scores = np.empty((individuals.shape[0], self._n_scores),  dtype=np.float64)
        received = 0
